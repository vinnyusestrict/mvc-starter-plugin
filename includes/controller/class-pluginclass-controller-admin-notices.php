...
class PluginClass_Controller_Admin_Notices {
    ...
    public function __call( $name = '', $args = array() ) {
        ...
        if ( preg_match( '/^add_(success|error|warning|info)$/', $name, $matches ) ) {
            ...
            if ( isset( $args['scope'] ) ) {
                if ( ! in_array( $args['scope'], array( 'global', 'user' ) ) ) {
     ...
     * 
     * @param array $notice     The notice array comprised of the following:
     *                          message, type, is_dismissible, screen_id, scope and cap
     */
    private function render_notice( $notice ) {
        ...
        if ( $is_dismissible ) {
            ...
            if ( isset( $this->dismissed_notices['global'][ $md5 ] ) || isset( $this->dismissed_notices['global'][ $md5 ] ) ) {
...
<?php
    ...
    private function load_dismissed() {
        ...
        $this->dismissed_notices = array('global' => $global, 'user' => $user );
    }
    ...
    public function add_dismissible_js() {
        ...
        if ( $this->has_dismissible ) {
            wp_enqueue_script( 'plugin-slug_admin_dismissible_notice', PluginClass()->environment->js_url . 'dismissible_notices.js', array('jquery'), PluginClass()->version );
        }
    }
    ...
    public static function uninstall() {
        ...
        $wpdb::delete(
            $wpdb->usermeta,
            array( 'meta_key' => 'PluginClass_notices' ),
            array( '%s' )
        );
    }
